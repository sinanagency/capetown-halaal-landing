...
function StatTile({ icon: Icon, value, label, href, accent }: { icon: LucideIcon; value: string; label: string; href: string; accent?: boolean }) {
  return (
    <a href={href} className={`group rounded-2xl p-4 border transition-colors ${accent ? 'bg-[#1a1416] border-[#1a1416] text-white' : 'bg-white border-neutral-200 hover:border-[#cd2653]/40'}`}>
      <Icon className={`w-4 h-4 mb-3 ${accent ? 'text-[#ff7a9c]' : 'text-[#cd2653]'}`} />
      <p className={`text-xl font-bold leading-tight ${accent ? 'text-white' : 'text-neutral-900'}`}>{value}</p>
      <p className={`text-xs mt-0.5 ${accent ? 'text-white/60' : 'text-neutral-500'}`}>{label}</p>
    </a>
  )
}
...
export default async function Overview() {
  ...
  const steps = [
    ...
    { done: staffCount > 0, label: 'Register your gate staff', sub: staffCount ? `${staffCount} on the manifest` : 'Names, ID, vehicle', href: '/exhibitor/portal/staff' },
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-7">
      {/* hero */}
      <div>
        <p className="text-xs uppercase tracking-[0.22em] text-[#cd2653] font-semibold">Exhibitor portal</p>
      ...
      <div className="grid md:grid-cols-[248px_1fr] gap-4">
        <div className="bg-white border border-neutral-200 rounded-2xl p-6 flex flex-col items-center justify-center text-center">
          <Gauge pct={readiness} label={`${readiness}%`} sublabel="show-ready" />
          <p className="text-sm font-medium text-neutral-700 mt-4">{doneCount} of {steps.length} steps done</p>
          <p className="text-xs text-neutral-400 mt-0.5">Finish the checklist below</p>
        </div>
      ...
      <div className="bg-white border border-neutral-200 rounded-2xl p-6">
        <p className="font-semibold text-neutral-900 mb-5">Your journey to show day</p>
            ...
            return (
              <div key={s} className="flex-1 flex items-center last:flex-none">
                <div className="flex flex-col items-center gap-1.5">
                  <div className={`w-9 h-9 rounded-full flex items-center justify-center text-sm font-semibold ${done ? 'bg-[#cd2653] text-white' : current ? 'bg-[#cd2653]/10 text-[#cd2653] ring-2 ring-[#cd2653]' : 'bg-neutral-100 text-neutral-400'}`}>
      ...
      <div className="grid md:grid-cols-5 gap-6">
        <div className="md:col-span-3 bg-white border border-neutral-200 rounded-2xl p-6">
          ...
          <ul className="space-y-1">
            {steps.map((a) => (
              <li key={a.label}>
                <a href={a.href} className="flex items-start gap-3 -mx-2 px-2 py-2 rounded-lg hover:bg-neutral-50 group">
                  {a.done ? <CheckCircle2 className="w-5 h-5 text-[#cd2653] mt-0.5 shrink-0" /> : <Circle className="w-5 h-5 text-neutral-300 mt-0.5 shrink-0" />}
                  ...
                  {!a.done && <ArrowRight className="w-4 h-4 text-neutral-300 group-hover:text-[#cd2653] mt-0.5" />}
                </a>
              </li>
            ))}
          </ul>
          ...
          <div className="bg-white border border-neutral-200 rounded-2xl p-5">
            ...
            {announcement ? (
              <a href="/exhibitor/portal/announcements" className="block group">
                <p className="font-semibold text-neutral-900 text-sm group-hover:text-[#cd2653]">{announcement.title}</p>
                <p className="text-sm text-neutral-600 mt-1 line-clamp-3">{announcement.body}</p>
                <p className="text-xs text-neutral-400 mt-2">{new Date(announcement.created_at).toLocaleDateString('en-ZA', { day: 'numeric', month: 'long' })}</p>
              </a>
            ) : (
              <p className="text-sm text-neutral-500">No announcements yet. Festival updates appear here.</p>
            )}
          </div>
          <div className="bg-white border border-neutral-200 rounded-2xl p-5 space-y-3 text-sm">
            <div className="flex items-start gap-3"><Calendar className="w-4 h-4 text-[#cd2653] mt-0.5" /><div><p className="font-medium text-neutral-900">11–13 December 2026</p><p className="text-neutral-500 text-xs">Youngsfield Military Base</p></div></div>
          </div>
        </div>
      </div>
    </div>
