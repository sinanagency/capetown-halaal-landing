...
export async function GET(request: NextRequest) {
  ...
  const rows = [['Business', 'Stall', 'Staff name', 'ID number', 'Vehicle reg', 'Application status'].map(esc).join(',')]
  ...
  for (const a of apps || []) {
    ...
    for (const m of state.staff || []) {
      rows.push([a.business_name, stall || '', m.name, m.id_number, m.vehicle_reg, a.status].map(esc).join(','))
    }
  }
